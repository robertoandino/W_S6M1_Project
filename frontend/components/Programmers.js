...
export default function Programmers() {
  ...
  const [awesomeList, setAwesomeList] = useState(listOfAwesome) //eslint-disable-line
  const [id, setId] = useState(0)
  const isCelebrating = id;
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredDev = awesomeList.find(dev => dev.id === id);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: isCelebrating ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          awesomeList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} 
              <button onClick={() => { setId(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
